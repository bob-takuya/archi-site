...
const config = {
  from: "chunks" as const,
  // URL to the database (separate into chunks)
  databaseUrl: '/db/archimap.sqlite',
  // Using public URLs instead of import.meta
  workerUrl: '/sqlite.worker.js',
  wasmUrl: '/sql-wasm.wasm',
  // For large databases, we need to split it into chunks
  httpRequestChunkSize: 1024 * 1024,
};

// TypeScriptに対応したワーカー設定
...
// Database query execution with caching support
export const queryDatabase = async (
  ...
  try {
    ...
    const cacheKey = `${sql}_${JSON.stringify(values)}`;
    
    // Try to get results from cache for SELECT queries
    if (
    ...
    try {
      // Call the query method on the worker
    ...
    // Cache SELECT query results if they're not too large
    if (
      sql.trim().toLowerCase().startsWith('select') && 
      rows.length < 500
    ) {
