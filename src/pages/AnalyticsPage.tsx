...
const AnalyticsPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        // Load architectures
        const archResult = await database.executeQuery<Architecture>(
          'SELECT * FROM ZARCHITECTURE ORDER BY ZAR_YEAR DESC LIMIT 5000'
        );
        
        if (archResult.success) {
          setArchitectures(archResult.data);
          setFilteredArchitectures(archResult.data);
        }

        // Load architects
        const archtsResult = await database.executeQuery<Architect>(
          'SELECT * FROM ZARCHITECT ORDER BY ZAR_NAME LIMIT 1000'
        );
        
        if (archtsResult.success) {
          setArchitects(archtsResult.data);
        }
