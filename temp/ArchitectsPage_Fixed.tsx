...
import { Architect, ArchitectsResponse, Tag } from '../types/architect';
...
// Architect data transformation helper
...
type TransformedArchitect = ReturnType<typeof transformArchitectData>;

const ArchitectsPage: React.FC = () => {
  console.log("ArchitectsPage コンポーネントがレンダリングされました");
  
  // 状態変数
  const [architects, setArchitects] = useState<TransformedArchitect[]>([]);
  ...
  const [sortBy, setSortBy] = useState<string>('ZAR_NAME'); // Use actual DB field names
  ...
  // 並び替えオプション - Use DB field names
  const sortOptions: SortOption[] = [
    { value: 'ZAR_NAME', label: '名前順', icon: <SortByAlphaIcon /> },
    { value: 'ZAR_BIRTHYEAR', label: '生年順', icon: <CalendarMonthIcon /> },
    { value: 'ZAR_NATIONALITY', label: '国籍順', icon: <PublicIcon /> }
  ];
  ...
  useEffect(() => {
    ...
    const loadArchitects = async () => {
      ...
      try {
        ...
        const sort = queryParams.get('sortBy') || 'ZAR_NAME';
        ...
        // 建築家データを取得 - Fixed parameter passing
        ...
        // Transform data for UI consumption - Fixed response structure
  ...
  // タグの年度取得関数 - Fixed database field names
  ...
  const updateUrlAndSearch = () => {
    ...
    if (sortBy !== 'ZAR_NAME') newQueryParams.set('sortBy', sortBy);
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Paper sx={{ p: 3, mb: 4 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 2, mb: 3, alignItems: 'flex-start' }}>
          ...
          {/* 並び替え - Fixed to use DB field names */}
          <FormControl sx={{ minWidth: 200 }}>
            ...
            >
              <MenuItem value="ZAR_NAME_asc">名前 (昇順)</MenuItem>
              <MenuItem value="ZAR_NAME_desc">名前 (降順)</MenuItem>
              <MenuItem value="ZAR_BIRTHYEAR_asc">生年 (昇順)</MenuItem>
              <MenuItem value="ZAR_BIRTHYEAR_desc">生年 (降順)</MenuItem>
              <MenuItem value="ZAR_NATIONALITY_asc">国籍 (昇順)</MenuItem>
              <MenuItem value="ZAR_NATIONALITY_desc">国籍 (降順)</MenuItem>
            </Select>
          </FormControl>
        </Box>
