import { executeSimpleQuery, executeSimpleQuerySingle } from './SimpleDatabase';
...
export const getArchitectureById = async (id: number): Promise<Architecture | undefined> => {
  return executeSimpleQuerySingle<Architecture>(
...
export const getArchitecturesByArchitect = async (architectId: number): Promise<Architecture[]> => {
  return executeSimpleQuery<Architecture>(
...
export const getAllArchitectures = async (
  ...
  try {
    // 総件数取得
    const countResult = await executeSimpleQuerySingle<{ total: number }>(
      `SELECT COUNT(*) as total FROM ZCDARCHITECTURE WHERE ${whereClause}`,
      params
    );
    const total = countResult?.total || 0;
    
    // データ取得
    const architectures = await executeSimpleQuery<Architecture>(
      `SELECT * FROM ZCDARCHITECTURE WHERE ${whereClause} ORDER BY ${orderByClause} LIMIT ? OFFSET ?`,
      [...params, limit, offset]
    );
...
export const getPrefectureCounts = async (): Promise<{ prefecture: string; count: number }[]> => {
  return executeSimpleQuery<{ prefecture: string; count: number }>(`
...
export const getArchitectureForMap = async (
  ...
  return executeSimpleQuery<Architecture>(
    `SELECT * FROM ZCDARCHITECTURE WHERE ${whereClause} LIMIT 500`,
    params
  );
};